...
class Rule extends \Magento\Rule\Model\ResourceModel\AbstractResource
{
    ...
    {
        $this->_init('emipro_smartproductselector_rules', 'rule_id');
    }
    ...
    protected function _afterDelete(\Magento\Framework\Model\AbstractModel $rule)
    {
        ...
        $connection->delete(
            $this->getTable('emipro_smartproductselector_products'),
            ['rule_id=?' => $rule->getId()]
        );
    ...
    {
        $data = $this->getRulePrices($date, $gId, [$pId]);
        if (isset($data[$pId])) {
            return $data[$pId];
        }
     ...
     * @return array
     */
    public function getRulePrices(\DateTime $date, $customerGroupId, $productIds)
    {
        $connection = $this->getConnection();
     ...
     * @return array
     */
    public function getRulesFromProduct($date, $productId)
    {
        $connection = $this->getConnection();
        if (is_string($date)) {
            $date = strtotime($date);
